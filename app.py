...
def generate_invoice_docx(invoice_data):
    ...
    # Define styles based on your Word document if possible
    # For exact replication, manual inspection of Word document styles is needed
    # For now, we'll use direct formatting for simplicity.

    # --- Header Section ---
    # Assuming "TAX INVOICE/DELIVERY NOTE" is bold and centered
    title = document.add_paragraph('TAX INVOICE/DELIVERY NOTE', style='Normal')
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run_title = title.runs[0]
    run_title.bold = True
    run_title.font.size = Pt(14) # Adjust font size if needed

    # Add a blank line for spacing
    document.add_paragraph()

    # Table for Contact and Invoice Details (To, Date, SSS Invoice No etc.)
    # Based on your format: To:, Date:, Address:, SSS Invoice No:, etc.
    # It looks like a 2x4 or 2x5 table structure for these details, then another table for ATTN etc.
    # For simplicity initially, let's use two columns and fill in.
    # Exact layout might require nested tables or more complex cell merging.
    ...
    table_header_data = [
        ...
        ["ATTN:", invoice_data['attn_person'], "", ""], # ATTN and Email are on new lines but under previous block
        ["Email:", invoice_data['customer_email'], "", ""],
        ["Customer PO#:", invoice_data['customer_po'], "", ""]
    ]

    # Create a table (adjust column widths as needed based on visual inspection of your doc)
    # This will create a 4-column table, which can be formatted.
    table = document.add_table(rows=len(table_header_data), cols=4)
    table.autofit = False # Disable autofit to control widths manually
    table.allow_autofit = False # Newer versions

    # Set column widths for better alignment (these are approximate, adjust as needed)
    # You'll need to fine-tune these by trial and error based on your DOCX
    col_widths = [Cm(2.5), Cm(6.5), Cm(3), Cm(6)] # Example widths
    ...
    for r_idx, row_data in enumerate(table_header_data):
        for c_idx, cell_text in enumerate(row_data):
            ...
            # Apply basic font size (e.g., 10pt)
    ...
    # --- Line Items Table ---
    # Define table headers from your document
    # "No", "Description", "Unit price", "BHD", "Qty", "Total price", "BHD"
    # This looks like 7 columns.
    ...
    # Set column widths for line items table (adjust carefully)
    ...
    for i, header_text in enumerate(item_table_headers):
        ...
        # Apply bold and background color if your template has it
        for paragraph in cell.paragraphs:
            for run in paragraph.runs:
                run.bold = True
                run.font.size = Pt(9)
        # Example: Apply a light gray shading if desired
        # cell.shading.background = RGBColor(0xD9, 0xD9, 0xD9) # Light gray

    # Add item rows
    for i, item in enumerate(invoice_data['line_items']):
        row_cells = item_table.add_row().cells
        row_cells[0].text = str(i + 1) # No.
        row_cells[1].text = item['description']
        row_cells[2].text = f"{item['unit_price']:.3f}" # Format to 3 decimal places for BHD
        row_cells[3].text = "BHD"
        row_cells[4].text = str(item['quantity'])
        row_cells[5].text = f"{item['total_price']:.3f}" # Format to 3 decimal places for BHD
        row_cells[6].text = "BHD"

        # Apply formatting to item rows (e.g., align numbers right)
        ...
        for j in [0, 1, 2, 3, 4, 5, 6]:
            for paragraph in row_cells[j].paragraphs:
                for run in paragraph.runs:
                    run.font.size = Pt(9) # Smaller font for line items

    # --- Summary Section (Subtotal, VAT, Grand Total) ---
    # This also looks like a table for alignment
    # Based on your template, these are right-aligned.
    summary_table = document.add_table(rows=3, cols=2)
    summary_table.autofit = False
    summary_table.allow_autofit = False

    summary_col_widths = [Cm(15.5), Cm(4.5)] # Adjust to align with main table
    ...
    # Make Grand Total bold and potentially a different color/size as in your template
    ...
    for p in grand_total_row[1].paragraphs:
        for run in p.runs:
            run.bold = True
            run.font.size = Pt(11)
            # run.font.color.rgb = RGBColor(0x00, 0x00, 0xFF) # Example: Blue color
    ...
    document.add_heading('Terms and Conditions', level=2) # Adjust heading level
    terms_para = document.add_paragraph()
    terms_para.add_run("Payment terms: ").bold = True
    terms_para.add_run(invoice_data['payment_terms']) # Dynamic payment terms
    ...
    # Using tabs or a small table for alignment might be better here for exact spacing
    received_by_para.add_run("\t\t   (Name)\t\t   (Date)\t          (Signature)").font.name = 'Courier New' # Using monospace for alignment
    # For more robust alignment, a table with one row and three cells is better.
    # For now, let's just add the static text and manager name.
...
# Use st.form for clear input boundaries and single submission
with st.form("invoice_form"):
    ...
    with col2:
        ...
        # Generate a simple SSS Invoice No (e.g., SSS-YYMMDD-001)
        # In a real app, this would come from a database or sequence generator
    ...
    # Dynamic line items - Use st.session_state to manage the list of items
    if 'line_items' not in st.session_state:
        st.session_state.line_items = []

    # Button to add new line item
    if st.button("Add New Item"):
        st.session_state.line_items.append({"description": "", "unit_price": 0.000, "quantity": 1})
    ...
    if submitted:
        if not st.session_state.line_items:
            ...
            invoice_data = {
                ...
                "invoice_date": invoice_date.strftime("%d-%m-%Y"), # Format date for document
